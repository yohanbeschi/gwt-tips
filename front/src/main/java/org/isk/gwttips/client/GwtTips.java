...
import org.isk.gwttips.client.util.StringUtils;
...
public class GwtTips implements EntryPoint {
     ...
     * @param errorMessage is the message to display.
     */
    ...
    /**
     * If the user connection succeed, hide the login form and display a message
     * with the first name and last name of the user.
     * @param firstname
     * @param lastname
     */
    ...
    /**
     * Handle the login for; submission after the user pressed Enter or click on the submit button.
     */
    private void handleSubmit() {
        ...
        if (login == null || login.isEmpty() || password == null || password.isEmpty()) {
            GwtTips.this.setErrorMessage(ERROR_CONSTANTS.connectionBlankFields());
        } else {
            this.connectionService.connect(login, password, new AsyncCallback<UserUI>() {
                @Override
                public void onFailure(Throwable caught) {
                    final String errorMsg = ERROR_CONSTANTS.getString(StringUtils.camelCase(caught.getMessage(), '.'));
                ...
                public void onSuccess(UserUI result) {
                    ...
                    GwtTips.this.errorBox.setVisible(false);
